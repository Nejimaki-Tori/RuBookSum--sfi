...
\begin{document}
\raggedbottom
\author{Denis\,A.~Grigoriev\footnote{\label{msu}Lomonosov Moscow State University, Research Computing Center, Moscow, Russia}\orcidID{0009-0007-2675-001X} \and Daniil\,V.~Khudiakov\footnoteref{msu} \and Daniil\,I.~Chernyshev\footnoteref{msu}\orcidID{0009-0001-6847-2122}}
...
\begin{abstract}
The majority of existing Russian document summarization datasets focus on short-form source documents which does not require complex causal analysis or 
coreference resolutions. Furthermore, processing longer multi-page texts poses a serious challenge to current generation of language models as the 
...
we propose optimized algorithms for long-document summarization, which enable up to 300\% summary generation speed up without significant drops in quality.
...
Our dataset contains high-quality human-written summaries for documents of different domains including fiction, popular science, children books and classical literature.
...
To address the lack of resources, a new dataset was created, using "Narodny Briefly" platform \cite{Briefly} where users publish summaries for popular books.
...
\fig{width=0.76\linewidth}{figures/genres_pie.png}{Distribution of texts by genres (top 10; "Other" aggregates all remaining genres)}

The genre distribution in the collection is shown in \figref{figures/genres_pie.png} and \tabref{tab:dataset_info} gives dataset statistics versus analogs.
...
However, literature often contains chunks that have little impact on plot development or contains redundant information without any additional details. 
...
Chunks that are close in cosine similarity to previous ones are considered redundant and are excluded from compilation of the summary at the current level.
...
Moreover, generating an outline at every step slows the method down and consumes extra computational time.
...
\textbf{BERTScore} \cite{bertscore}. For every token pair from prediction and reference we compute cosine similarity of their embeddings. Then:
\begin{equation}
...
\textbf{Key question coverage (Coverage)} is the proportion of questions that are answered in reference that are covered in generated summary:
\begin{equation}
...
\textbf{Factual agreement (Agreement)} is the average cosine similarity between answers $a_i^{\text{pred}}$ generated based on predicted summary and answers $a_i^{\text{ref}}$ obtained from reference summary to the same questions from Coverage metric:
\begin{equation}
...
To obtain embeddings, we used USER-bge-m3 model\footnoteref{encoder}. 
To generate questions and answers for Coverage and Agreement metrics, we use Qwen3-235B-A22B \cite{qwen3} model. 
...
We selected these models to cover three comparable size tiers and to ensure strong Russian capability. 
...
In all tables models are grouped by parameter count: within each size group, the best result is underlined, and the overall best is in bold type. 
Values are mean $\pm$ SD across test documents. Metrics of automatic book summarization across models and methods are shown in \tabref{tab:results_models}.
...
\scalebox{0.87}{
  \begin{tabular}{|p{0.15\linewidth}|p{0.75\linewidth}|}
    ...
    \makecell{\textbf{Kastrjuk}} & \dots \uline{The story takes place in a Russian village in early spring, 
...
In the first case the model scored high, answering all but one question, but the other summary contained answers to only two questions out of eleven, leading to a low score. 
These two summaries are shown in \tabref{tab:refs}. For brevity only the main points that affected the final metric were highlighted.
...
In the texts, bold type marks plot-relevant fragments, while underlined type indicates content that could be omitted.
...
To confirm the efficiency of proposed algorithm modifications and to measure the actual speed up, we conducted an isolated test using text "1408" by Stephen King. 
...
than some 32B models achieving almost a 300\% speed up on some stories.
...
\section*{Conclusion}
In this work we introduced RuBookSum, the first open dataset for Russian long-narrative summarization.
...
However, for certain text types and methods (e.g., baseline blueprint), more compact models such as RuadaptQwen3-32B-Instruct-v2 can be a competitive cost-efficient alternative.
...
Outcomes can vary with many factors: the choice of models, the composition of the test set (which specific books, genres, and lengths are included in), etc.
...
If $\theta$ were too small, too many fragments would be filtered out, risking loss of useful content, and if too large, most fragments would be kept, defeating the purpose of filtering.
...
The work was carried out using the "MSU-270" supercomputer of the Lomonosov Moscow State University.
